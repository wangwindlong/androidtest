...
public final class BluetoothDevicePreference extends GearPreference {
    ...
    void onClicked() {
        ...
        if (mCachedDevice.isConnected()) {// 已连接，询问是否断开连接
            ...
            askDisconnect();
        } else if (bondState == BluetoothDevice.BOND_BONDED) {// 已绑定，则进行连接
            metricsFeatureProvider.action(context,
                    SettingsEnums.ACTION_SETTINGS_BLUETOOTH_CONNECT);
            mCachedDevice.connect();
        } else if (bondState == BluetoothDevice.BOND_NONE) { // 如果未绑定，则进行配对
            metricsFeatureProvider.action(context,
